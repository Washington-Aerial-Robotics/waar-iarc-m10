...
void peripheral_dw3000Init() {
  memory mempage = { "dw3000", sizeof( dw ), &dw };
  firmware_registerPeripheral( { 0, true, &mempage, &peripheral_dw3000Init, &peripheral_dw3000Loop } );
  DPRINTF( "[P] Initializing DW3000\n" );
  dw.working = false;
  dw.frameLength = 0;
  dw.broadcastIndex = 0;
  dw.coms.currentTime = 0;
  dw.coms.method = DW_COM_METHOD;
  dw.coms.allowBroadcast = true;
  dw.coms.fwdReply = true;
  dw.coms.packet = dw.rx.raw;
  dw.coms.receiving = []() { return ( unsigned short )dw.frameLength; };
  dw.coms.replying = []( void* buffer, unsigned short len ) { 
  ...
  dw.coms.sending = []( void* buffer, unsigned short len ) {
