...
void peripheral_serialInit() {
  memory mempage = { "serial", sizeof( serial.coms ), &serial.coms };
  firmware_registerPeripheral( { 0, true, &mempage, &peripheral_serialInit, &peripheral_serialLoop } );
  DPRINTF( "[P] Initializing Serial\n" );
  serial.coms.currentTime = 0;
  serial.coms.allowBroadcast = false;
  serial.coms.fwdReply = true;
  serial.coms.method = SERIAL_COM_METHOD;
  serial.coms.packet = COMS_BUFFER;
  serial.coms.receiving = &serialReceiving;
  serial.coms.replying = &serialSending;
  serial.coms.sending = &serialSending;
}
