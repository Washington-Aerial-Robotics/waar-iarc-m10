...
void com_receiveMessage( const STDBYTE type, const unsigned char minContentSize, 
  ...
  if( i < RECEIVE_FUNC_COUNT ) {
    DPRINTF( "[C] Registering Receive Type=%02x, Size=%u\n", type, minContentSize );
...
static void* replyMessage( const comContent* rx, STDBYTE* messageType, unsigned short* messageSize ) {
  switch( *messageType ) {
    ...
    case COM_SET_TRIGGER : {
      kafenv.info.trigger = rx->triggervalue;
    ...
    case COM_SET_FLIGHTMODE : case COM_SET_INFO : case COM_SET_STEST : case COM_SET_MOTORS : case COM_SET_CALIB : case COM_SET_KAFENV : {
      *messageSize = 0;
      *messageType = COM_SUCCESS;
...
memory com_reset() {
  ...
  coms.contentSizes[COM_SET_TRIGGER]     += sizeof( coms.tx.content.triggervalue );
  ...
  coms.contentSizes[COM_SET_FLIGHTMODE]  += sizeof( coms.tx.content.flightmode );
  ...
  memory mempage = { "communication", sizeof( coms ), &coms };
  return mempage;
}
    ...
    if( rxheader.toID == kafenv.info.deviceID ) {
      switch( rxheader.messageType ) {
        ...
        case COM_SET_FLIGHTMODE : {
          kafenv.info.flightMode = content->flightmode;
          FPFILL0( q, kafenv.cmd.setpoints );
          DPRINTF( "[C] Process Message: Received=SET_FLIGHTMODE, Mode=%02x\n", kafenv.info.flightMode );
          break;
        }
        case COM_SET_INFO : {
