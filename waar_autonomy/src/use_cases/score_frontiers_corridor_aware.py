...
Ranks unvisited frontier blocks by a score that combines:
  - proximity to goal (reverse-Dijkstra cost from goal)
...
def best_frontier(
    ...
    if reserved_frontiers:
        fronts = [b for b in fronts if b not in reserved_frontiers]
...
def _reverse_dijkstra(world: WorldModel, unknown_cost: float) -> np.ndarray:
    """Dijkstra backwards from goal_fine; impassable cells get np.inf."""
