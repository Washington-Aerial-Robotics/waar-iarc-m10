...
public:
    void operator()( matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs ) {
        ...
        switch( ( unsigned char )command[0] ) {
            ...
            case 'S' : {
                pidtuner_step( &bridge_sensor );
                flight_step( &bridge_sensor );
                double stateData[13];
                pindtuner_simulationState( stateData );
                TypedArray<double> state = factory.createArray<double>( { 13, 1 } );
                for( unsigned char i = 0; i < 13; i++ ) state[i] = stateData[i];
                outputs[0] = state;
                break;
            }
            case 'E' : {
                TypedArray<double> currentTime = inputs[1];
                commander_step( ( unsigned long )currentTime[0] );
                outputs[0] = factory.createScalar( true );
                break;
            }
            default : { }
